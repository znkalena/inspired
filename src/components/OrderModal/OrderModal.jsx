...
export const OrderModal= () => {
    const {order:{values,id,order,totalPrice}} =useSelector(state => state.cart);
            ...
            <div className={s.modal} onClick={handleCloseModal}>
                <div className={s.body} onClick={handleModalclick}>
                    ...
                    <ul className={s.customer}>
                        ...
                        {values.delivery==='delivery' ? 'Доставка' :'Самовывоз'}</span>
                    </li>
                    </ul>
                    <ul className={s.goods}>
                        {order.map(item =>{
                            const product =goodsList.find(good =>good.id===item.id);
                            return(
